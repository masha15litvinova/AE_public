...
ServoDriverSmooth servo(0x40, 180); // с указанием адреса и макс. угла
uint32_t myTimer;
void setup() {
  Serial.begin(9600);
  servo.setMaxAngle(180);
  servo.attach(8, 275, 825);  // 150 и 600 - длины импульсов, при которых
...
void loop() {
  ...
  if (millis() - myTimer >= 40) {
    myTimer = millis();
    int newPos = 0; // берём с потенцометра значение 0-180
